...
An example of the usefulness of this technique is the retrieval of Variables from Vizzy programs by using `FlightProgram` (or `VZ` for short) for the Modifier Selector, followed by the name of the variable. If the variable is a vector the prefix `v:` will be required. All parts have very useful properties you can access, they are all listed in [this page](PartModifierScriptProperties).